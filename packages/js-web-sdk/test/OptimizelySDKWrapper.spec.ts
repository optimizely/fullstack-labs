...
function testPublicApi(
  userId: string,
  createOptimizelyFn: () => Promise<OptimizelySDKWrapper>,
): void {
  ...
  describe('getVariation', function() {
    ...
    it('should allow getVariation(expKey, overrideUserId, overrideAttributes)', function() {
      ...
      const result = optimizely.getVariation(
        testName,
        overrideUserId,
        overrideAttributes,
      )
  ...
  describe('isFeatureEnabled', function() {
    ...
    it('should allow isFeatureEnabled(featureKey, overrideUserId, overrideAttributes)', function() {
      ...
      const result = optimizely.isFeatureEnabled(
        featureKey,
        overrideUserId,
        overrideAttributes,
      )
  ...
  describe('getFeatureVariableString', function() {
    ...
    it('should allow getFeatureVariableString(featureKey, variableKey, overrideUserId)', function() {
      ...
      const result = optimizely.getFeatureVariableString(
        featureKey,
        'stringVar',
        overrideUserId,
      )
      ...
      sandbox.assert.calledOnce(optimizely.instance
        .getFeatureVariableString as sinon.SinonSpy)
      sandbox.assert.calledWithExactly(
  ...
  describe('getFeatureVariableBoolean', function() {
    ...
    it('should allow getFeatureVariableBoolean(featureKey, variableKey, overrideUserId)', function() {
      const overrideUserId = 'jon'
      const result = optimizely.getFeatureVariableBoolean(
        featureKey,
        variableKey,
        overrideUserId,
      )
      ...
      sandbox.assert.calledOnce(optimizely.instance
        .getFeatureVariableBoolean as sinon.SinonSpy)
      sandbox.assert.calledWithExactly(
  ...
  describe('getFeatureVariableInteger', function() {
    ...
    it('should allow getFeatureVariableInteger(featureKey, variableKey, overrideUserId)', function() {
      const overrideUserId = 'jon'
      const result = optimizely.getFeatureVariableInteger(
        featureKey,
        variableKey,
        overrideUserId,
      )
      ...
      sandbox.assert.calledOnce(optimizely.instance
        .getFeatureVariableInteger as sinon.SinonSpy)
      sandbox.assert.calledWithExactly(
  ...
  describe('getFeatureVariableDouble', function() {
    ...
    it('should allow getFeatureVariableDouble(featureKey, variableKey, overrideUserId)', function() {
      const overrideUserId = 'jon'
      const result = optimizely.getFeatureVariableDouble(
        featureKey,
        variableKey,
        overrideUserId,
      )
      ...
      sandbox.assert.calledOnce(optimizely.instance
        .getFeatureVariableDouble as sinon.SinonSpy)
      sandbox.assert.calledWithExactly(
  ...
  describe('getFeatureVariables', function() {
    ...
    it('should allow getFeatureVariables(featureKey, overrideUserId)', function() {
      ...
      sandbox.assert.calledThrice(optimizely.instance
        .getFeatureVariableString as sinon.SinonSpy)
      sandbox.assert.calledWithExactly(
        optimizely.instance.getFeatureVariableString as sinon.SinonSpy,
        featureKey,
    ...
    it('should allow getFeatureVariables(featureKey, overrideUserId, overrideAttributes)', function() {
      ...
      const result = optimizely.getFeatureVariables(
        featureKey,
        overrideUserId,
        overrideAttributes,
      )
  ...
  describe('getForcedVariation', function() {
    ...
    it('should allow getForcedVariation(experimentKey, overrideUserId)', function() {
      ...
      sandbox.assert.calledOnce(optimizely.instance
        .getForcedVariation as sinon.SinonStub)
      sandbox.assert.calledWithExactly(
  ...
  describe('setForcedVariation', function() {
    ...
    it('should allow setForcedVariation(experimentKey, overrideUserId, variationKey)', function() {
      const overrideUserId = 'joe'
      const result = optimizely.setForcedVariation(
        experimentKey,
        overrideUserId,
        variationKey,
      )
      sandbox.assert.calledOnce(optimizely.instance
        .setForcedVariation as sinon.SinonStub)
      sandbox.assert.calledWithExactly(
      ...
      const getForcedVariationResult = optimizely.getForcedVariation(
        experimentKey,
        overrideUserId,
      )
...
describe('OptimizelySDKWrapper blackbox tests', function() {
  ...
  describe('when passing config to instantiation', function() {
    this.beforeEach(function() {
      sandbox.stub(optimizelySDK, 'createInstance').returns({
        notificationCenter: {},
      } as any)
    })
  ...
  describe('initializing with `sdkKey` and `userId`', function() {
    const userId = 'bob'
    ...
    describe('when requesting datafile responds 200', function() {
      ...
      it('should allow usage of the NotificationCenter', async function() {
        const optimizely: OptimizelySDKWrapper = getControlledOutputSDKWrapper({
          sdkKey,
        })
        assert.isFalse(optimizely.isInitialized)
        ...
        optimizely.activate('single_variation_abtest', 'user1')
        sinon.assert.calledOnce(activateSpy)
      })
    ...
    describe('when requesting the datafile is delayed', function() {
      ...
      describe("when the client isn't initialized", function() {
        ...
        it('getFeatureVariableX should return null', function() {
          ...
          assert.isFalse(optimizely.isInitialized)
          assert.isNull(
            optimizely.getFeatureVariableString(
          ...
          assert.isNull(
            optimizely.getFeatureVariableBoolean(
              'variable_test_feature',
              'booleanVar',
              userId,
            ),
          ...
          assert.isNull(
            optimizely.getFeatureVariableInteger(
              'variable_test_feature',
              'integerVar',
              userId,
            ),
          ...
          assert.isNull(
            optimizely.getFeatureVariableDouble(
              'variable_test_feature',
              'doubleVar',
              userId,
            ),
        ...
        it('should allow initialization of a Noop NotificationCenter', async function() {
          const optimizely: OptimizelySDKWrapper = getControlledOutputSDKWrapper({
            sdkKey,
          })
          assert.isFalse(optimizely.isInitialized)
          ...
          optimizely.notificationCenter.clearNotificationListeners(
            'ACTIVATE:experiment, user_id,attributes, variation, event' as any,
          )
